...
const knownPrefix = "<hbe-prefix></hbe-prefix>";
  ...
  function hbeLoader() {
    ...
    if (passInput) {
      passInput.setAttribute('enterkeyhint', 'done');
    }

    async function unlockByPassword() {
      if (!cryptoObj || !cryptoObj.subtle) {
        alert('当前浏览器不支持密码解锁，请使用系统浏览器（Safari/Chrome）打开。');
        return;
      }
    ...
    if (passInput) {
      ...
      // Some mobile keyboards don't emit reliable Enter key events.
