...
const knownPrefix = "<hbe-prefix></hbe-prefix>";
  ...
  async function decrypt(decryptKey, iv, hmacKey, silent = false) {
  ...
  function hbeLoader() {
    ...
    async function tryUnlock(password, silent = false) {
      if (!password || isDecrypting || isUnlocked) return false;
      isDecrypting = true;
      try {
    ...
    if (oldStorageData) {
      ...
      cryptoObj.subtle.importKey('jwk', sDk, {
        ...
        cryptoObj.subtle.importKey('jwk', sHmk, {
          ...
          decrypt(dkCK, sIv, hmkCK, true).then((result) => {
            if (!result) {
    ...
    if (passInput) {
      passInput.addEventListener('input', async () => {
        // Auto-unlock once the final correct character is entered.
        await tryUnlock(passInput.value, true);
      });
    }
    ...
    mainElement.addEventListener('keydown', async (event) => {
      if (event.isComposing || event.keyCode === 13) {
        ...
        await tryUnlock(password, false);
      }
    });
  }
