(() => {
  ...
  const storageTTL = 1000 * 60 * 60 * 24 * 30; // 30 days
...
const knownPrefix = "<hbe-prefix></hbe-prefix>";
  ...
  async function verifyContent(key, content, silent) {
  ...
  async function decrypt(decryptKey, iv, hmacKey, silent) {
    ...
    const result = await cryptoObj.subtle.decrypt({
      ...
      return await verifyContent(hmacKey, decoded, silent);
    }).catch((e) => {
      if (!silent) {
  ...
  function hbeLoader() {
    ...
    let oldStorageData = null;
    try {
      oldStorageData = JSON.parse(storage.getItem(storageName));
    } catch (e) {
      storage.removeItem(storageName);
    }
    if (oldStorageData && oldStorageData.ts && (Date.now() - oldStorageData.ts > storageTTL)) {
      storage.removeItem(storageName);
      oldStorageData = null;
    }
    ...
    async function tryDecrypt(silent) {
      try {
        const password = document.getElementById('hbePass').value;
        if (!password) {
          if (status) status.textContent = '请输入密码后再解锁。';
          return;
        }
        ...
        const result = await decrypt(decryptKey, iv, hmacKey, silent);
        ...
        if (status) status.textContent = '解锁失败，请刷新页面后重试。';
        console.log(e);
      }
    }
    ...
    if (passInput) {
      // add a manual unlock button and status text for better feedback
      let unlockBtn = document.getElementById('hbeUnlockBtn');
      if (!unlockBtn) {
        unlockBtn = document.createElement('button');
        unlockBtn.id = 'hbeUnlockBtn';
        unlockBtn.type = 'button';
        unlockBtn.className = 'hbe-button';
        ...
        passInput.parentNode && passInput.parentNode.appendChild(unlockBtn);
      }

      status = document.getElementById('hbeStatus');
      if (!status) {
        ...
        status.textContent = '';
        passInput.parentNode && passInput.parentNode.appendChild(status);
      }
      ...
      passInput.addEventListener('input', () => {
        ...
        warnTimer = setTimeout(() => {
          if (!unlocked) {
            status.textContent = '未解锁，请检查密码或点击“解锁”按钮。';
          }
        }, 1200);
      });
    }
